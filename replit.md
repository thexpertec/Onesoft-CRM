...
| 5 | Shipped (May 2026) | **products** — bridge flipped on `admin-products` → `products`. Frontend infrastructure (`productsApi`, `_saveProducts` chokepoint, `_persistProductRest`, `_dualWriteProductsDiff`, all 8 `setStored(PRODUCTS_KEY,…)` callsites rerouted, dedup/orphan branches in `syncAllFromServer` Step 4b extended to call `_persistProductRest("delete",…)` for kv-bridge tenants, `syncProductsToStore` rewritten to use `productsApi.update` for tenant scope) was already in place but the bridge entry was previously reverted because the Neon-cutover left the relational `products` table empty. Backfilled this batch via a new **`POST /api/admin-backup/backfill-products`** one-shot endpoint (idempotent, ON CONFLICT DO NOTHING): 156 rows inserted across 12 tenants. Bridge flipped on after the backfill ran cleanly. Bridge GET and per-record GET now return identical counts (verified 78=78 on the largest tenant). |
| Full-DB backup/restore | Shipped (May 2026) | New superadmin-only feature in Settings → Data Management: **Download Full DB** dumps every public table to JSON via `GET /api/admin-backup/download` (information_schema enumeration, all tables — kv_store + 47 relational); **Restore Full DB** uploads the dump and rewrites the entire DB via `POST /api/admin-backup/restore` in a single transaction with `SET LOCAL session_replication_role = replica` (FK ordering bypass) + TRUNCATE CASCADE per table + drift-tolerant per-row INSERT (filters to currently-existing columns, JSON-stringifies jsonb values). UI: red AlertDialog confirmation, format magic-string `_format=onesoft-db-backup-v1`. Gated UI-side on `isSuperAdmin`; backend trusts the global `X-Api-Key` (admin-dashboard sends it). Use case: migrate a whole superadmin deployment to another instance, or take a forensic snapshot before risky operations. |
...
| 5 | Shipped (May 2026) | **products + staff** — catalogue + HRM entities. Both use the simple `makeRecordApi<T>(path)` factory (no child tables — jsonb on row). `_saveProducts` / `_saveStaff` chokepoints introduced; all 8 `setStored(PRODUCTS_KEY,…)` callsites (createProduct/updateProduct/deleteProduct/bulkReplaceProductImages/reorderProducts/PO-receive/Mfg-output/SKU-backfill migration) and all 8 `setStored(STAFF_KEY,…)` callsites (createStaff/updateStaff/deleteStaff/COA-heal/m11-backfill/role-rename/desig-rename/m14-clear) rerouted through them. Bridge entries `"admin-products": "products"` + `"admin-hrm-staff": "staff"` registered. **Three bypass sites fixed during architect review** (products only): `syncProductsToStore` uses per-record `productsApi.update` for tenant scope; `bulkImportProducts` calls `_saveProducts(finalList)` + adds an **awaited** `_persistProductsAwait(prior, next, tid)` to `writes[]` so CSV import keeps its "products durable on resolve" contract (fire-and-forget would lose writes on fast reload); `syncAllFromServer` Step 4b dedup fires per-record `_persistProductRest("delete")` for removed IDs in tenant scope. Same anti-recursion invariant as Batch 4b — the single inner `setStored` per chokepoint is the only one for each key in the file. Recursion was caught & fixed during the staff sed-replace, same as Batch 4 Bug #1. **Re-ship attempted + rolled back May 2026** (Neon migration follow-up): `MIGRATED_KEY_TO_TABLE` had been emptied during the Neon cutover. Attempted to re-register the bridge entry in `kv.ts`, but the relational `products` table was never backfilled from `kv_store` during the cutover (all 13 tenants' products still live exclusively in `kv_store` — Techeaven alone has 565). Flipping the bridge on made every tenant's product list show 0 because the bridge short-circuits the `kv_store` read. **Reverted within minutes** — `MIGRATED_KEY_TO_TABLE` is empty again with a header comment warning future maintainers not to re-enable any key until the corresponding relational table is backfilled from `kv_store`. The `nullifyUndefined` fix in `_persistProductRest` + `_persistProductsAwait` is **kept** — it's defensive correctness for the dual-write path and harmless while the bridge is off. **Open follow-up**: write a `scripts/backfill-relational.ts` that reads each migrated key from `kv_store`, inserts every row into the per-record table (one INSERT per element of the JSON array), then re-enable the bridge entries one at a time with manual verification per tenant. |
