...
function NewPurchaseReturnSheet({ onClose, onSaved }: NewReturnSheetProps) {
  ...
  const handleSelect = (inv: Invoice) => {
    ...
    // Build already-returned map: sku (or productName) → total qty returned via prior posted returns
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      // Post JE
      const je = autoPostPurchaseReturnJE({
      ...
      if (je) updatePurchaseReturn(pr.id, { jeId: je.id });

      toast({ title: `${pr.returnNumber} posted`, description: `Stock adjusted · ${je ? "JE posted" : "JE skipped (configure COA accounts in Settings)"}` });
      onSaved();
    } catch (e: unknown) {
