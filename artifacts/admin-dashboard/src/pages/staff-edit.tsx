...
import { Users2, Save, ArrowLeft } from "lucide-react";
...
export default function StaffEditPage() {
  ...
  const member = useMemo(() => staff.find(s => s.id === params.id), [staff, params.id]);
  ...
  const [form, setForm] = useState(() => ({
    ...
    email:          member?.email        ?? "",
    phone:          member?.phone        ?? "",
    joinDate:       member?.joinDate     ?? format(new Date(), "yyyy-MM-dd"),
    salaryType:     (member?.salaryType  ?? "Monthly") as "Monthly" | "Hourly" | "Daily" | "Commission",
    basicSalary:    member?.basicSalary  != null ? String(member.basicSalary)  : "",
    allowances:     member?.allowances   != null ? String(member.allowances)   : "",
    deductions:     member?.deductions   != null ? String(member.deductions)   : "",
    bankName:       member?.bankName     ?? "",
    accountNumber:  member?.accountNumber ?? "",
    openingBalance: member?.openingBalance != null ? String(member.openingBalance) : "",
    notes:          member?.notes        ?? "",
  }));
  ...
  if (!member) {
    return (
      <div className="max-w-4xl mx-auto py-20 text-center">
        ...
        <Button variant="outline" onClick={() => nav("/staff")} className="mt-4">Back to Staff</Button>
      </div>
    );
  }
  ...
  const handleSave = () => {
    ...
    toast({ title: "Changes saved", description: `${form.name.trim()} has been updated.` });
    nav("/staff");
  };
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
        ...
        <div className="flex items-center gap-4 px-6 py-5 bg-gradient-to-r from-blue-600 to-indigo-600">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-[16px] font-bold text-white leading-snug">Edit Staff Member</h1>
        ...
        <div className="px-6 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Field label={`Basic Salary (${sym})`} hint="Base pay">
            ...
            <Field label={`Allowances (${sym})`} hint="Transport, housing…">
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground pointer-events-none">{sym}</span>
                <Input type="number" step="0.01" min="0" placeholder="0.00"
            ...
            <Field label={`Deductions (${sym})`} hint="Tax, provident fund…">
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground pointer-events-none">{sym}</span>
                <Input type="number" step="0.01" min="0" placeholder="0.00"
            ...
            <Field label={`Net Pay (${sym})`} hint="Calculated automatically">
              <div className={`h-9 rounded-md border px-3 flex items-center text-sm tabular-nums font-bold ${
                net >= 0
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Field label={`Opening Balance (${sym})`} hint="Salary advance / balance owed">
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground pointer-events-none">{sym}</span>
