...
const COLS: ColDef[] = [
  ...
  { field: "product",               label: "Product",             minW: 150, type: "text"   },
  { field: "business",              label: "Business",            minW: 150, type: "text"   },
  { field: "specificProductGroups", label: "Product Groups",      minW: 160, type: "text"   },
...
export default function InvestmentPlansPage() {
  ...
  const commitCell = useCallback((id: string, field: EditableField, value: string) => {
    ...
    if (!p || (p as unknown as Record<string, string>)[field] === value) { setActiveCell(null); return; }
    editPlan(id, { [field]: value } as Partial<InvestmentPlan>);
    setActiveCell(null);
    toast({ title: "Saved" });
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        {isAuthenticated && (
          <div className="flex items-center gap-2 flex-wrap">
            {plans.length === 0 && !exampleLoaded && (
              <Button size="sm" variant="outline" onClick={loadExamples} className="gap-1.5 h-8 text-[13px] border-dashed">
                Load Examples
              </Button>
            )}
            ...
            return (
              <tr key={plan.id} data-testid={`row-plan-${plan.id}`}
                ...
                {COLS.map((c, ci) => {
                  ...
                  // Special display for Locked Period
                  const isLockCol = c.field === "lockForSpecificTime";
                  // Special display for profit/loss margin columns — colour-coded
                  ...
                  return (
                    <td key={c.field}
                      ...
                      onClick={() => !isA && isAuthenticated && setActiveCell({ id: plan.id, col: ci })}>

                      {!isA && isLockCol && rawVal ? (
