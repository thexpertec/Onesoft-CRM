...
function InvoicePanel({ invoice, onClose, onSave, onDelete, onStatusChange, onCollectPayment, defaultType = "sale" }: PanelProps) {
  ...
  return (
    <div className="-mx-5 md:-mx-8 -my-6 md:-my-8 min-h-full bg-gray-50 dark:bg-zinc-950 flex flex-col">
              ...
              <div className="px-5 py-4 space-y-3">
                ...
                <div>
                  ...
                  <textarea rows={2} value={form.buyerAddress} onChange={e => setF("buyerAddress", e.target.value)} placeholder="Street address…"
                    className="w-full px-3 py-2 rounded-none border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 outline-none resize-none"/>
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="date" value={form.dueDate} onChange={e => setF("dueDate", e.target.value)}
                      className={`w-full px-3 py-2 rounded-none border bg-white dark:bg-zinc-800 text-sm focus:ring-2 focus:ring-blue-500 outline-none ${isOverdue(form as unknown as Invoice) ? "border-red-300 dark:border-red-800 text-red-600 dark:text-red-400" : "border-gray-200 dark:border-zinc-700 text-gray-900 dark:text-gray-100"}`}/>
                  </div>
                </div>
                {/* Sales Agent */}
                  ...
                  <div className={`grid ${invoiceType !== "purchase" ? "grid-cols-[28px_1fr_110px_80px_110px_100px_36px_156px]" : "grid-cols-[28px_1fr_110px_80px_110px_100px_36px]"} gap-0 px-4 py-3 items-center transition-colors ${isDelivered ? "bg-emerald-50/40 dark:bg-emerald-950/10" : "hover:bg-gray-50/50 dark:hover:bg-zinc-800/30"}`}>
                    ...
                    <div className="pl-1 pr-2" data-item-product={item.id}>
                      {isDelivered ? (
                        ...
                        <Combobox value={item.productName} onChange={v => pickProduct(item.id, v)}
                          ...
                          inputClassName="w-full px-2.5 py-1.5 rounded-none border border-gray-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-sm text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 outline-none"/>
                      )}
                      {item.localName && (
                    ...
                    <div className="px-1">
                      <input type="number" min="0" value={item.qty}
                        ...
                        className={`w-full px-2 py-1.5 rounded-none border text-sm text-center outline-none transition-colors ${
                          isDelivered
                    ...
                    <div className="px-1 flex gap-1">
                      <input type="number" min="0" {...(item.discountType !== "amt" ? { max: "100" } : {})} step="0.01"
                        ...
                        className="w-full px-2 py-1.5 rounded-none border border-gray-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-sm text-center text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 outline-none"/>
                      <button type="button"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" min="0" step="0.01" value={form.shippingFee} onChange={e => setF("shippingFee", e.target.value)} placeholder="0.00"
                      className="w-full pl-7 pr-3 py-2.5 rounded-none border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 outline-none"/>
                  </div>
                </div>
                <div>
      ...
      {/* ── Revert Inventory confirm ─────────────────────────────────────── */}
      <AlertDialog open={revertStockOpen} onOpenChange={setRevertStockOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Revert Inventory?</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <p>For <strong className="text-gray-900 dark:text-gray-100">{invoice?.invoiceNumber}</strong>, this will:</p>
                <ul className="list-disc list-inside space-y-1 pl-1">
                  <li>Restore all deducted quantities back to inventory</li>
                  <li>Reset Sale Status to <strong>Pending</strong></li>
                </ul>
                <p className="text-sm text-gray-500 dark:text-gray-400">Payments and journal entries are not affected.</p>
                <p className="text-amber-600 dark:text-amber-400 font-medium">This cannot be undone.</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (invoice) {
                  revertInvoiceStock(invoice.id);
                  toast({ title: "Inventory reverted", description: "Stock restored to inventory · Sale status reset to Pending" });
                }
                setRevertStockOpen(false);
              }}
      ...
      {/* ── Revert Payments confirm ──────────────────────────────────────── */}
      <AlertDialog open={revertPaymentsOpen} onOpenChange={setRevertPaymentsOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Revert Payments?</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <p>For <strong className="text-gray-900 dark:text-gray-100">{invoice?.invoiceNumber}</strong>, this will:</p>
                <ul className="list-disc list-inside space-y-1 pl-1">
                  <li>Delete all cash-receipt journal entries (RCPT-*)</li>
                  <li>Reset all linked payment vouchers to draft</li>
                  <li>Clear payment history and outstanding balance</li>
                  <li>Roll invoice status back to <strong>Sent</strong></li>
                </ul>
                <p className="text-sm text-gray-500 dark:text-gray-400">Stock and the original accrual journal entry are not affected.</p>
                <p className="text-rose-600 dark:text-rose-400 font-medium">This cannot be undone.</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (invoice) {
                  revertInvoicePayments(invoice.id);
                  toast({ title: "Payments reverted", description: "Receipt JEs deleted · vouchers reset to draft · invoice back to Sent" });
                }
                setRevertPaymentsOpen(false);
              }}
                  ...
                  <div>
                    {inv.saleStatus ? (
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${
                        isPurchase
                          ? inv.saleStatus === "Received"
                            ? "bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300"
                            : inv.saleStatus === "Partially Received"
                              ? "bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300"
                              : inv.saleStatus === "Cancelled"
                                ? "bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400"
                                : "bg-purple-100 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300"
                          : inv.saleStatus === "Delivered" || inv.saleStatus === "Completed"
                            ? "bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300"
                            : inv.saleStatus === "Partially Delivered"
                              ? "bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300"
                            : inv.saleStatus === "Dispatched"
