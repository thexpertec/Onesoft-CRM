...
import {
  ...
  FlaskConical, FileText, Package, Settings2, TruckIcon,
  Globe, Store, Flag, BarChart3, ArrowLeft,
} from "lucide-react";
import {
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white dark:bg-card rounded-xl border border-border overflow-hidden">
      <div className="px-5 py-3 border-b border-border bg-gray-50 dark:bg-muted/30">
...
export default function RepairReportPage() {
  ...
  // Timeline: jobs per day (all time within range)
  ...
  // Aging: open jobs older than N days
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5 print:p-0 print:space-y-3">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3 print:hidden">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Analytics & detailed log of all repair bookings</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <KpiCard label="Total Jobs"      value={total}          icon={Wrench}       color="bg-blue-600"    sub={`${dateFrom} → ${dateTo}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Section title="Jobs by Priority">
          {byPriority.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={byPriority} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} innerRadius={40} paddingAngle={3} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
          ...
          <div className="flex flex-col gap-4">
            {sourceData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={sourceData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} paddingAngle={4}
            ...
            <div className="grid grid-cols-2 gap-3">
              {([["Online", Globe, "bg-sky-100 text-sky-700 dark:bg-sky-900/40"],
                 ["Shop Visitor", Store, "bg-violet-100 text-violet-700 dark:bg-violet-900/40"]] as const).map(([label, Icon, cls]) => (
        ...
        {timeline.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={timeline} margin={{ top: 4, right: 16, bottom: 4, left: -16 }}>
              ...
              <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
              <Tooltip content={<ChartTip />} />
              <Area type="monotone" dataKey="count" name="Jobs" stroke="#3b82f6" strokeWidth={2} fill="url(#repairGrad)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </Section>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="py-16 text-center text-muted-foreground">
              ...
              <p className="text-sm font-medium">No jobs in this date range</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y divide-border">
                {sorted.map((b, i) => {
                  ...
                  const sCls   = {
                    ...
                    "Diagnosing":"text-violet-700 bg-violet-50 border-violet-200",
                    "Quoted":"text-indigo-700 bg-indigo-50 border-indigo-200",
                    "Awaiting Parts":"text-orange-700 bg-orange-50 border-orange-200",
                    "In Repair":"text-amber-700 bg-amber-50 border-amber-200",
                    "Ready":"text-teal-700 bg-teal-50 border-teal-200",
                  ...
                  return (
                    <tr key={b.id} className="hover:bg-blue-50/20 dark:hover:bg-blue-950/10 transition-colors">
                      ...
                      <td className="px-3 py-2.5">
                        ...
                        <span className="block text-[10px] text-muted-foreground/60 truncate max-w-28">{b.tenantId}</span>
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-xs text-foreground/80 max-w-40">
                      ...
                      <td className={`px-3 py-2.5 text-xs tabular-nums text-right ${ageCls}`}>
                        {done ? <span className="text-emerald-600">✓</span> : age}
                      </td>
                      <td className="px-3 py-2.5 text-xs text-foreground/70 max-w-48">
