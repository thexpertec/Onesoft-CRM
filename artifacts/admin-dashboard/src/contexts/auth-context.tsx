...
// Impersonation and tab-tenant-override are also in sessionStorage (tab-scoped by design).
const AUTH_KEY        = "onesoft-admin-auth";
const AUTH_USER_ID    = "onesoft-admin-user-id";
const TENANT_KEY      = "onesoft-tenant-id";
const IMPERSONATE_KEY = "onesoft-impersonate-from";
...
function restoreActiveTenant(): string | null {
  try {
    const raw = _ss.getItem(TENANT_KEY);
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      if (tenant) {
        ...
        setActivityUser(tenantUser.fullName || tenantUser.username);
        _ss.setItem(AUTH_KEY,     "true");
        _ss.setItem(AUTH_USER_ID, `tenant:${tenant.id}`);
        _ss.setItem(TENANT_KEY,   tenant.id);
        setCurrentUser(tenantUser);
  ...
  const loginAs = async (
    ...
    const origUserId = _ss.getItem(AUTH_USER_ID) ?? "";
    _ss.setItem(IMPERSONATE_KEY, origUserId);
    ...
    if (as === "admin") {
      const tenantUser = tenantToAdminUser(tenant);
      setActivityUser(tenantUser.fullName || tenantUser.username);
      _ss.setItem(AUTH_USER_ID, `tenant:${tenantId}`);
      _ss.setItem(TENANT_KEY, tenantId);
      setCurrentUser(tenantUser);
    ...
    if (as === "staff" && memberId) {
      ...
      setActiveTenant(tenantId);
      setActivityUser(staffUser.fullName || staffUser.username);
      _ss.setItem(AUTH_USER_ID, `staff:${memberId}`);
      _ss.setItem(TENANT_KEY, tenantId);
      setCurrentUser(staffUser);
      setCurrentTenantId(tenantId);
      setIsImpersonating(true);
    ...
    if (as === "sales_agent" && memberId) {
      ...
      setActiveTenant(tenantId);
      setActivityUser(agentUser.fullName || agentUser.username);
      _ss.setItem(AUTH_USER_ID, `agent:${memberId}`);
      _ss.setItem(TENANT_KEY, tenantId);
      setCurrentUser(agentUser);
      setCurrentTenantId(tenantId);
      setIsImpersonating(true);
    ...
    _ss.removeItem(IMPERSONATE_KEY);
    return false;
  };

  // ── Exit Impersonation ─────────────────────────────────────────────────────
  const exitImpersonation = () => {
    const origUserId = _ss.getItem(IMPERSONATE_KEY);
    if (!origUserId) return;

    _ss.removeItem(IMPERSONATE_KEY);
    setActiveTenant(null);
    _ss.setItem(AUTH_USER_ID, origUserId);
    _ss.setItem(TENANT_KEY, "");
  ...
  const logout = () => {
    ...
    _ss.removeItem(AUTH_USER_ID);
    _ss.removeItem(TENANT_KEY);
    _ss.removeItem("onesoft_tab_tenant");
    _ss.removeItem(IMPERSONATE_KEY);
    setCurrentUser(null);
    setCurrentTenantId(null);
  };
