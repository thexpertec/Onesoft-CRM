...
router.get("/tenant/:tenantId/status", async (req, res, next) => {
  try {
    ...
    const [invoicesRow] = await query<{ count: string }>(
    ...
    const [rpVouchersRow] = await query<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM rp_vouchers WHERE tenant_id = $1 AND archived_at IS NULL`,
      [tenantId],
    );
    const [staffRow] = await query<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM staff WHERE tenant_id = $1 AND archived_at IS NULL`,
      [tenantId],
    );
    ...
    const [kvInvoicesRow] = await query<{ value: unknown }>(
    ...
    const [kvRpVouchersRow] = await query<{ value: unknown }>(
      `SELECT value FROM kv_store WHERE namespace = $1 AND key = 'admin-rp-vouchers' LIMIT 1`,
      [`t:${tenantId}`],
    );
    const [kvStaffRow] = await query<{ value: unknown }>(
